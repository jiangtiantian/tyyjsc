...
public class GoodsViewPagerAdapter extends PagerAdapter {
    ...
    private ArrayList<GoodsBriefIntroduction> infors;

    private int size;

    public void setInfors(ArrayList<GoodsBriefIntroduction> infors) {
        this.infors = infors;
    }

    public GoodsViewPagerAdapter(Context mContext, ArrayList<GoodsBriefIntroduction> infors,
                                 View view) {
    ...
    //获取每页展示的数据
    public ArrayList<GoodsBriefIntroduction> getEachData(int position) {
        ArrayList<GoodsBriefIntroduction> data = new ArrayList<>();
        if (getCount() == 1) {
            data.addAll(infors);
        } else {
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        if (container.getChildAt(position) == null) {
            ...
            GoodsGridViewAdapter adapter = new GoodsGridViewAdapter(mContext, getEachData(position));
        ...
        GoodsGridViewAdapter adapter = (GoodsGridViewAdapter) holder.gridView.getAdapter();
        if (adapter == null) {
            adapter = new GoodsGridViewAdapter(mContext, getEachData(position));
            holder.gridView.setAdapter(adapter);
        } else {
